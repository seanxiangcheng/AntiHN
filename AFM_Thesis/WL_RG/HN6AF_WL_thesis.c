...
double Logfnow=1.0, Logfmin=0.00000005, MinStateLogg, MaxSteps=2.0e13;
...
// Check if neighbors array already has neighbor site n
// if yes, return 0; if no, return 1;
int Check_Repeat_Neig(int neighbors[], int n)
{
    int i = 0;
    for(i=0; i < MAXNEIG; i++)
    {
...
unsigned int *Findij(int n){
  static unsigned int ij[2];
  ij[0]=1;
  ij[1]=0;
  while(n%2==0) {
    n=n/2;
...
int * FindNeighbors(int n){ 
  	static int neighbors[MAXNEIG];
  	unsigned int *ij;
  	unsigned int i,index;
  	unsigned int ni0, nj0, ni1,ni2, n1,n2;
  	if(n>L || n==0){
  	...
  	if(ni0==1){	//  if level 1
            ...
            n2 = n-Intpow(2, ni0-1);
            if(n2 == 0) n2 = L;
            if(Check_Repeat_Neig(neighbors, n1))
                neighbors[index++]=n1;	// connecting the 2 sites 2^(i-1) away
            if(Check_Repeat_Neig(neighbors, n2))
                neighbors[index++]=n2;
            return(neighbors);   
  	}
  	else if(ni0<K){
            ...
            {
      			for(i=0;i<(ni0-2);i++)
                {	// sites nn that may connect to site n by connecting back and forth; 
        			n1=n-3*Intpow(2,i);
        			n2=n+3*Intpow(2,i);
        			...
        			if(ni1==i+1 && Check_Repeat_Neig(neighbors, n1)){
          				neighbors[index++]=n1;
        			}
        
        			if(ni2==i+1 && Check_Repeat_Neig(neighbors, n2)){
          				neighbors[index++]=n2;
        			}
      			}
                n1 = n+Intpow(2, ni0-1);
                ...
                {	// sites nn that may connect to site n by connecting back and forth; 
        			...
        			if(ni1==i+2 && Check_Repeat_Neig(neighbors, n1))
          				neighbors[index++]=n1;
        
        			if(ni2==i+2 && Check_Repeat_Neig(neighbors, n2))
          				neighbors[index++]=n2;
      			}  
    		}
    		return(neighbors);  
            ...
            for(i=0;i<(ni0-2);i++){
        		...
        		if(ni2==i+2 && n2!=n1 && Check_Repeat_Neig(neighbors, n2)){
            				neighbors[index++]=n2;
        		}
     		}
            return(neighbors);          
...
int main(int argc, char *argv[]){
  	...
  	char dos_file[20]="H6AFDOS";	// Name of the output file of density of states
  	char buf[8];
    ...
    int neighbors[7*L+8], neighloc[L+1];

    Initial_Steps= (double)((double)K*20*(double)L*(double)L);
